...
public class InformeCuaderno {
	...
	public InformeCuaderno(String dni) throws SQLException {
		DatosUsuarioParcela(dni);
		this.Dni = dni;
		crear_PDF("Informe Cuaderno", "Opengis", "", "");

	}

	public void DatosUsuarioParcela(String dni) {
	...
	public void crear_PDF(String t, String a, String s, String k)
		...
		if (this.ruta_destino != null) {
			try {
				...
				PdfWriter.getInstance(mipdf,
						new FileOutputStream(this.ruta_destino + ".pdf"))
						.setInitialLeading(6);
				...
				Paragraph n = new Paragraph("OPENGIS", FontFactory.getFont(
						"arial", 22, Font.BOLD));
				n.setAlignment(Element.ALIGN_CENTER);
				Paragraph n1 = new Paragraph(year, FontFactory.getFont("arial",
						30, Font.BOLD));
				n1.setAlignment(Element.ALIGN_CENTER);
				Paragraph n2 = new Paragraph(
						"CUADERNO DE EXPLOTACIÓN PARA CULTIVO SOSTENIBLE ",
						FontFactory.getFont("arial", 18, Font.BOLD));
				n2.setAlignment(Element.ALIGN_CENTER);
				Paragraph n3 = new Paragraph("REGLAMENTO(CEE)Nº 1698/2005",
						FontFactory.getFont("arial", 16, Font.BOLD));
				n3.setAlignment(Element.ALIGN_CENTER);
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				mipdf.add(n);
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("1. IDENTIFICACIÓN DEL PRODUCTOR",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(
						"Nombre y Apellidos/Empresa: " + nombre, FontFactory
								.getFont("arial", 13, Font.BOLD)));
				mipdf.add(new Paragraph("D.N.I./N.I.F.:" + Dni, FontFactory
						.getFont("arial", 13, Font.BOLD)));
				mipdf.add(new Paragraph(
						"Identificación del Responsable Técnico:" + linea,
						FontFactory.getFont("arial", 13, Font.BOLD)));
				mipdf.add(new Paragraph(
						"Cooperativa o SAT de la que forma parte:" + linea,
						FontFactory.getFont("arial", 13, Font.BOLD)));
				mipdf.add(new Paragraph("2. IDENTIFICACIÓN DE LAS PARCELAS",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" "));
				...
				int contador = 0;
				String g;
				int[] x = { 0, 7, 15, 23, 31, 39, 47, 55, 63, 71, 79, 87, 95,
						103, 111, 119, 127, 135, 143, 151, 159, 167, 175, 183,
						191, 199, 207, 215, 223 };
				while (rs.next()) {
					if (x[contador] == y) {
						g = "" + contador;
				...
				mipdf.add(tabla);
				mipdf.add(new Paragraph(
						"(1) La superficie se reflejará en hectáreas con dos decimales.",
						FontFactory.getFont("arial", 8, Font.BOLD)));

				mipdf.newPage(); // tercera pagina

				mipdf.add(new Paragraph(
						"3. PERIODO DE INUNDACIÓN ANTES DE LA SIEMBRA",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("4. PREPARACION DEL TERRENO",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph("Fangueado o fresadora", FontFactory
						.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(tabla2);
				mipdf.add(new Paragraph(
						"(2) Hacer constar el nº de orden de todas las parcelas si se agrupan.",
						FontFactory.getFont("arial", 8, Font.BOLD)));

				mipdf.newPage(); // cuarta pagina

				mipdf.add(new Paragraph("Laboreo", FontFactory.getFont("arial",
						14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("Nivelación (en su caso)", FontFactory
						.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("5. SIEMBRA", FontFactory.getFont(
						"arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph("Fangueado o fresadora", FontFactory
						.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
				...
				mipdf.add(new Paragraph(
						...
						FontFactory.getFont("arial", 8, Font.BOLD)));
				mipdf.add(new Paragraph(
						"(*) Especificar si se trata de semilla CERTIFICADA O SIN CERTIFICAR",
						FontFactory.getFont("arial", 8, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				mipdf.add(new Paragraph(
						"6. TRATAMIENTOS FITOSANITARIOS(Polvirizar)",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("7. ABONADO", FontFactory.getFont(
						"arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph(
						"(2) Hacer constar el nº de orden de todas las parcelas si se agrupan.",
						FontFactory.getFont("arial", 8, Font.BOLD)));
				mipdf.add(new Paragraph("8. COSECHA", FontFactory.getFont(
						"arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(new Paragraph("9.DETERMINACIONES ANALÍTICAS",
						FontFactory.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.add(tabla9);
				mipdf.add(new Paragraph("10.VISITAS TÉCNICAS", FontFactory
						.getFont("arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 22,
						Font.BOLD)));
				...
				mipdf.newPage();
				mipdf.add(new Paragraph("11. CIERRE", FontFactory.getFont(
						"arial", 14, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph(
						"En el presente Cuaderno de 'Cultivo sostenible' se han reflejado los datos de las operaciones de cultivo realizadas durante el periodo comprendido entre su apertura y el final de la campaña "
								+ year, FontFactory.getFont("arial", 12,
								Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph(
						"El titular del mismo da fe de que los datos reflejados son ciertos.",
				...
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph("SELLO OCAPA/DIRECCIÓN TERRITORIAL"
						+ espacio + espacio + espacio
						+ "El titular del cuaderno", FontFactory.getFont(
						"arial", 12, Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
				...
				mipdf.add(new Paragraph(" ", FontFactory.getFont("arial", 12,
						Font.BOLD)));
				mipdf.add(new Paragraph(espacio + "(Sello)" + espacio + espacio
						+ espacio + espacio
						+ "Fdo.  .....................................",
						FontFactory.getFont("arial", 12, Font.BOLD)));
