...
public class GeneradorPanelPrincipal extends JPanel {
	...
	public JButton getBActualizar() {
		if (bActualizar == null) {
			...
			bActualizar.setIcon(new ImageIcon(
					"OpenGis/src/recursosVisuales/actualizar.png")); //$NON-NLS-1$
	...
	public JTextField getTxtCriterioBusqueda() {
		if (txtCriterioBusqueda == null) {
			txtCriterioBusqueda = new JTextField(""); //$NON-NLS-1$
			...
			/*
			 * Acción de ratón. Cuando clic nos elimina el texto que hay en la
			 * caja de texto Así podemos trabajar de una forma mucho más limpia
			 * y eficaz.
			 */
	...
	public final JTable getTablaPrincipal() {
		if (tablaPrincipal == null) {
			tablaPrincipal = new JTable(modelo) {
				...
				public boolean isCellEditable(int row, int column) { // La tabla
																		// no
																		// será
																		// editable
					return false;
			...
			tablaPrincipal.addMouseListener(new java.awt.event.MouseAdapter() { // Cuando
																				// hagan
																				// clic...
						public void mouseClicked(java.awt.event.MouseEvent e) {
	...
	private JButton getBNuevo() {
		if (bNuevo == null) {
			...
			bNuevo.setIcon(new ImageIcon(
					"OpenGis/src/recursosVisuales/Nuevo.png")); //$NON-NLS-1$
	...
	public JButton getBModificar() {
		if (bModificar == null) {
			...
			bModificar.setIcon(new ImageIcon(
					"OpenGis/src/recursosVisuales/Modificar.png")); //$NON-NLS-1$
	...
	public JButton getBEliminar() {
		if (bEliminar == null) {
			...
			bEliminar.setIcon(new ImageIcon(
					"OpenGis/src/recursosVisuales/Eliminar.png")); //$NON-NLS-1$
			bEliminar.setEnabled(false);
	...
	/*
	 * Declaramos méotods para sustituirlos con polimorfismo en la clase que se
	 * herede
	 */
