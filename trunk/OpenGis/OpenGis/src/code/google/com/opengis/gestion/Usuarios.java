...
public class Usuarios {
	...
	public void crearUsuario() throws SQLException {
		...
		existe = false;
		activo = false;
		existe = ConectarDBA.comprobarExiste("usuario", "dni", this.dni, true); //$NON-NLS-1$ //$NON-NLS-2$
		System.out.println("Estado de existe en Usuarios = "+existe+" estado de activo = "+activo);
		if (existe == true) {
			...
			String sentencia = "INSERT INTO `dai2opengis`.`usuario` (`dni`, `nombre`, `apellidos`, `email`, `password`, `tipo`, `veces`, `telefono`, `direccion`, `poblacion`, `provincia`, `cp`, `fecha_nacimiento`, `activo`) VALUES ('" //$NON-NLS-1$
					+ this.dni + "', '" //$NON-NLS-1$
	...
	public void borrarUsuario() throws SQLException {
		...
		existe = ConectarDBA.comprobarExiste("usuario", "dni", this.dni, false); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void modificarUsuario() throws SQLException {
		...
		if (ConectarDBA.getExiste() == true) {
			...
			String sentencia = "UPDATE `dai2opengis`.`usuario` SET `Nombre` = '" + this.nombre + "', `Apellidos` = '" + this.apellidos + "', `Telefono` = '" + this.telefono + "',`Direccion` = '" + this.direccion + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					"',`Población` = '"
					+ this.poblacion
