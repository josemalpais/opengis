...
public class UsuariosPanelDatosPersonales extends JPanel {
	...
	public UsuariosPanelDatosPersonales(String dni, String nombre,
					...
					if (valido == true) {
						if (pass.equals(pass2)) {
							Usuarios u = new Usuarios(
									...
									txtTelefono.getText(),
									txtDireccion.getText(),
									comboPoblacion.getSelectedItem().toString(),
									txtProvincia.getText(), txtCP.getText(),
									txtFechaNac.getText(), pass, comboTipo
	...
	private JButton getBModificar() {
		if (bModificar == null) {
			...
			bModificar.setIcon(new ImageIcon(getClass().getResource(
					"/recursosVisuales/Modificar.png"))); //$NON-NLS-1$
			bModificar.setToolTipText(Idioma
					.getString("etModifyAccountSettings")); //$NON-NLS-1$
	...
	private JComboBox getComboPoblacion() {
		if (comboPoblacion == null) {
			...
			comboPoblacion.setEnabled(false);
	...
	private void activarCampos(boolean estado) {
		...
		txtFechaNac.setEnabled(estado); //$NON-NLS-1$
		txtDireccion.setEnabled(estado); //$NON-NLS-1$
		txtCP.setEnabled(estado); //$NON-NLS-1$
		comboPoblacion.setEnabled(estado);
		txtProvincia.setEnabled(estado); //$NON-NLS-1$
		txtTelefono.setEnabled(estado); //$NON-NLS-1$
		txtMail.setEnabled(estado); //$NON-NLS-1$
		txtPass.setEnabled(estado);//$NON-NLS-1$
		txtPass2.setEnabled(estado); //$NON-NLS-1$
