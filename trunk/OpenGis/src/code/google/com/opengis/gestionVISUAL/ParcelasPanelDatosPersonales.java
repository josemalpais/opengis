...
public class ParcelasPanelDatosPersonales extends GeneradorPanelPrincipal{
	...
	public void nuevo(){
		...
		ParcelasPanelNuevo p = null;
		try {
			p = new ParcelasPanelNuevo("alta");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}  //$NON-NLS-1$
		try {
			p.getComboUser().addItem(dniUsuario);
			p.getComboUser().setEnabled(false);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	// Recogemos todos los datos de la tabla
		...
		if(rParcela[8].toString().equals(Idioma.getString("etActive"))){
			...
			ParcelasPanelNuevo p = null;
			try {
				p = new ParcelasPanelNuevo("modificar",rParcela[0].toString(),rParcela[1].toString(),rParcela[2].toString(),rParcela[3].toString(),rParcela[4].toString(),rParcela[5].toString(),rParcela[6].toString(),rParcela[7].toString());
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} // Creamos el panel de Alta de Parcelas //$NON-NLS-1$
